...
export const GET: APIRoute = async ({ request }) => {
  ...
  const { data } = await supabaseAdmin.from('app_config').select('value').eq('key', 'mardi_flash_actif').single();
...
export const POST: APIRoute = async ({ request }) => {
  ...
  await supabaseAdmin.from('app_config').upsert({ key: 'mardi_flash_actif', value: actif ? 'true' : 'false', updated_at: new Date().toISOString() });
